...
> **2026-09-19 驗收補記**：本文件的 commit、行號與「尚未瀏覽器實測」是當時交接狀態，不代表現行程式全部仍未驗證。本次已在桌機瀏覽器操作四連桿、滑塊曲柄、齒輪對的選取、參數修改、播放／暫停及 3D。input 的吸附合併、雙指 pinch、手機完整互動等仍未由本批補驗，不能把整個 input 模組標為驗收完成。最新證據與限制以 [第一批收尾紀錄](CLOSEOUT-BATCH-1.md) 為準；保留以下歷史待辦，不據此自動擴大重構。
...
**仍未做 / 已知坑**：
- 拖去**合併到不在嚙合圓上的地錨**會脫咬：已加**防呆紅環**——`gearMeshOff(c)`（`gear-editor.js`）偵測「兩中心都接地但
...
- slice 2「draw/solver/compile 型別分派登錄表化」**進行中**：繪製分派表 `PART_DRAW[type] = { phase, draw }`
  放在 `app.js`（DOM 層，非純 part-types.js — 不碰 DOM 邊界）。`phase` 決定繪製時機：
  `'underlay'`＝畫在連桿之下、`'layered'`＝畫進 zlift 疊放層（用 ctx 取對應 `<g>`）。已填：
  - **slice 2a**：`gear`（`drawGearPart`，underlay）。
  - **slice 2b**：`triangle`（`drawTrianglePart`，layered，ctx 帶 groupForLayer/triLayerByKey/triKey）。
  兩刀皆**純抽取、行為位元級不變**（同碼、同呼叫點、同 z 序；三角板內部解構改名 a,b,d 以免遮蔽參數 c）。
  **其餘 draw 多非「按 comp 型別自繪」**：馬達走 compiled steps、連桿走 visualization、節點走 pts，
  不適合硬塞此表。compile（topology.js）與 solve（solveBodyJointTopology force-set）的型別分派**尚未表化**
  （那是「加機件＝加表項」的另一維，屬核心檔、需另議）。
